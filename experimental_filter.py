...
class OneEuroFilter(object):
    ...
    def __init__(self, freq: float, mincutoff: float = 1.0, beta: float = 0.0, dcutoff: float = 1.0, threshold: float = 1.0) -> None:
        ...
        :param threshold: Threshold for fast response to significant changes.
        ...
        if threshold < 0:
            raise ValueError("threshold should be >=0")
    ...
    def __call__(self, x: float, timestamp: float = None) -> float:
        ...
        if abs(x - prev_x) > self.__threshold:
            self.__x.reset()
            self.__x.__s = x
    ...
    def setParameters(self, freq: float, mincutoff: float = 1.0, beta: float = 0.0, dcutoff: float = 1.0, threshold: float = 1.0) -> None:
